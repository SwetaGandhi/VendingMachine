...
int main(int argc, char **argv) {
  ...
  TThreadedServer server(
      std::make_shared<PreferenceBeverageServiceProcessor>(
          std::make_shared<PreferenceBeverageServiceHandler>()),
  ...
  // 5: start the server
  std::cout << "Starting the preference beverage server ..." << std::endl;
  server.serve();
  return 0;
}
